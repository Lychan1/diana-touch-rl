...
本任务是整个项目的第一个 RL 里程碑。前 6 周完成了 URDF 清洗、Isaac Sim 部署、USD 资产转换、底层关节控制与观测接口打通，并封装出 `DianaEnv`；第 7 周正式设定"触碰目标方块"任务、跑通 PPO 训练闭环，**最终落地版本为 V8**。
...
代码里"距离"算的是方块质心，但物理引擎的碰撞体（Collider）不让手臂穿进方块表面——于是手臂再怎么努力，也永远到不了代码要求的"质心距离 < 阈值"。V8 把成功判定放宽到0.12m 阈值包含方块半径，并换成不会在近处爆炸的倒数型奖励，模型才收敛，让"触碰"在物理上真正可达、判定最贴合真实接触点。
